...
def start_message(message):
        ...
        bot.send_message(message.chat.id,'Привет, я бот, который покажет тебе нынешние курсы валют, поможет с переводом денежных валют')
...
def button_message(message):
    if message.text =="Начать" or message.text =="Назад":
        ...
        print(str(datetime.now())+" || " +str(message.from_user.id)+" || "+str(message.from_user.username)+" || " + message.text)
        bot.send_message(message.chat.id, "Введите валюты в формате: Исходная_валюта Значение Конечная_валюта (RUB - рубли)(Все валюты через буквенный код)")
        bot.register_next_step_handler(message,Money_Convert)
    elif message.text == "Конвертация иностранных валют":
        print(str(datetime.now())+" || " +str(message.from_user.id)+" || "+str(message.from_user.username)+" || " + message.text)
        bot.send_message(message.chat.id, "Введите валюты в формате: Исходная_валюта Значение Конечная_валюта (Все валюты через буквенный код)")
        bot.register_next_step_handler(message,Money_Convert)
        ...
        print(str(datetime.now())+" || " +str(message.from_user.id)+" || "+str(message.from_user.username)+" || " + message.text)
        bot.send_message(message.chat.id,"Введите валюту, изменение курса которой, вы хотите посмотреть (Валюта в виде буквенного кода)")
